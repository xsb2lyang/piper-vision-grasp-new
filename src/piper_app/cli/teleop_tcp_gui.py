...
from piper_app.teleop.gui import run
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Desktop GUI TCP pose teleop for Piper-series robotic arms."
    )
    parser.add_argument(
        "--robot",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Default TCP translation step in meters.",
    )
    parser.add_argument(
        "--rot-step-deg",
        type=float,
        default=teleop_defaults.get("rot_step_deg", 2.0),
        help="Default TCP rotation step in degrees.",
    )
    parser.add_argument(
        "--gripper-step",
        type=float,
        default=teleop_defaults.get("gripper_step_m", 0.005),
        help="Default gripper single-step width increment in meters.",
    )
    parser.add_argument(
        "--gripper-force",
        type=float,
        default=teleop_defaults.get("gripper_force_n", 1.0),
        help="Default gripper force in N.",
    )
    parser.add_argument(
        "--poll-interval-s",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=teleop_defaults.get("continuous_interval_ms", 120),
        help="Hold-to-move repeat interval in milliseconds.",
    )
    parser.add_argument(
        "--tcp-offset",
        type=float,
