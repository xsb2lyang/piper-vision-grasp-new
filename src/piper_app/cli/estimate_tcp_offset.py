...
from piper_app.tcp_offset.gui import run
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Estimate a flange-frame TCP translation offset by clicking a target and manually aligning the tool center."
    )
    parser.add_argument(
        "--robot",
        type=str,
    ...
    parser.add_argument(
        "--output-path",
        type=str,
        default=str(repo_root() / calibration_defaults["results_dir"] / "tcp_offset_estimate.yaml"),
    )
    parser.add_argument(
        "--session-base-dir",
        type=str,
        default=calibration_defaults["session_base_dir"],
        help="Base directory for saved tcp-offset estimation sessions.",
    )
