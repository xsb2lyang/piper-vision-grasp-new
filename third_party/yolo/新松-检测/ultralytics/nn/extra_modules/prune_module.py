...
from ..extra_modules.transformer import PSABlock_EPGO
...
class C2PSAEPGO_infer(nn.Module):
    """可剪枝版本的 C2PSAEPGO，使用独立卷积替代 split 操作"""

    def __init__(self, c1, c2, n=1, e=0.5):
        super().__init__()
        assert c1 == c2, 'Input and output channels must be equal'
        self.c = int(c1 * e)  # 中间通道数

        # 使用两个独立卷积替代 split 操作
        self.cv0 = Conv(c1, self.c, 1, 1)  # 分支 a
        self.cv1 = Conv(c1, self.c, 1, 1)  # 分支 b
        self.cv2 = Conv(2 * self.c, c1, 1)  # 合并分支

        # 使用 EPGO 优化的 PSA 块
        self.m = nn.Sequential(
            *(PSABlock_EPGO(self.c, attn_ratio=0.5, num_heads=self.c // 64) for _ in range(n))
        )

    def forward(self, x):
        a = self.cv0(x)  # 独立生成分支 a
        b = self.cv1(x)  # 独立生成分支 b
        b = self.m(b)  # 注意力处理
        return self.cv2(torch.cat([a, b], 1))  # 合并分支
...
class C2PSA_v2(nn.Module):
    def __init__(self, c1, c2, n=1, e=0.5) -> None:
        super().__init__()
        assert c1 == c2
        self.c = int(c1 * e)###跟infer不同的地方在于c1和c2不一样
        self.cv0 = Conv(c1, self.c, 1, 1)
        self.cv1 = Conv(c1, self.c, 1, 1)
        self.cv2 = Conv(2 * self.c, c1, 1)
