...
from ultralytics.nn.extra_modules.transformer import C2PSA_EPGO

# 没有购买yolov11项目需要注释以下
from ultralytics.nn.extra_modules.block import RepNCSPELAN4, C3k2_Faster_CGLU, C3k_Faster_CGLU, C3k2_MutilScaleEdgeInformationEnhance, C3k_MutilScaleEdgeInformationEnhance
...
def replace_c2f_with_c2f_v2(module):
    ...
    for name, child_module in module.named_children():
        if isinstance(child_module, C3k2):
            ...
            transfer_weights_c2f_to_c2f_v2(child_module, c2f_v2)
            setattr(module, name, c2f_v2)
        elif isinstance(child_module, C2PSA_EPGO):
            # Replace C2PSA_EPGO with C2PSA_EPGO_v2 while preserving its parameters
            # 注意：C2PSA_EPGO 的参数与 C2PSA 相同，所以我们可以用同样的方式创建 C2PSA_EPGO_v2
            shortcut = infer_shortcut(child_module.m[0])
            c2f_v2 = C2PSAEPGO_v2(child_module.cv1.conv.in_channels, child_module.cv2.conv.out_channels,
                                  n=len(child_module.m),
                                  e=child_module.c / child_module.cv2.conv.out_channels)
            transfer_weights_c2f_to_c2f_v2(child_module, c2f_v2)
            setattr(module, name, c2f_v2)
